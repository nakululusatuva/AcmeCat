		...
		OpensslWrap::AsymmetricRSA::PublicKeyList authorizedKeys)
{
	...
	auto isIPV4 = std::regex_match(address, std::regex(R"(::ffff:(\d+\.\d+\.\d+\.\d+)$)"));
	if (isIPV4)     // Accept incoming IPv4 connections on an IPv6 socket, reformat the padded ipv4 address, example: '::ffff:127.0.0.1' to '127.0.0.1'
		address = std::regex_replace(address, std::regex("::ffff:"), "");
