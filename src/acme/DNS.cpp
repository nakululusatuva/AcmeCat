...
std::shared_ptr<DNS::API> DNS::API::create(const std::string& providerName, const Json::Value& apiParams)
{
	...
	if ( !DNS::ProviderIsSupported(lowerCase) )
		...
		return std::make_shared<DNS::Cloudflare>(apiParams);
	else
		return nullptr;
}

DNS::Cloudflare::Cloudflare(const Json::Value& apiParams)
{
...
Json::Value DNS::Cloudflare::getAllRecords()
{
...
Json::Value DNS::Cloudflare::getRecordsAtPage(int pageNum)
{
	/* Append parameters to path */
...
Json::Value DNS::Cloudflare::apiResponseReformat(const Json::Value& response)
{
...
std::string DNS::Cloudflare::addTxtRecord(const std::string& name, const std::string& content)
{
	/* Append parameters to path */
...
void DNS::Cloudflare::deleteRecord(const std::string &recordId)
{
	/* Append parameters to path */
...
void DNS::Cloudflare::updateRecord(const std::string& recordId, const std::string& newName, const std::string& newType, const std::string &newContent)
{
	/* Append parameters to path */
	std::string path = std::regex_replace(urlPath, std::regex("\\?"), zoneId) + "/" + recordId;
